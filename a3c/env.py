...
class MecBCEnv1():
    def __init__(self):
        ...
        for k in range(1, np.math.floor(Total_PCG / Min_GroupBlock) + 1):
            pr = P(Total_PCG, k, BadNodeProb)
            # print(pr)
            if pr >= P_min and pr <= P_max:
                self.Kspace.append(k)
        # print(self.Kspace)

        self.SMspace = [0.5 * i for i in range(1, 9)]
        self.DIspace = [0.5 * i for i in range(1, 9)]
        self.state = [[0, 0] for i in range(Total_node + Total_user)]
        ...
        self.input_state.extend(list(inputt[-(Total_node + Total_user):]))
    ...
    def step(self, action):
        ...
        DataRate = [
            Total_spectrum / (Total_node + Total_user) * np.math.log(self.Sinr[self.state[i][1]] + 1) / np.math.log(2)
            for i in range(Total_user + Total_node)]
        DataRate = np.array(DataRate)
        DataRate_UserMin = np.min(DataRate[Total_node: Total_node + Total_user])
        DataRate_PCGMin = np.min(DataRate[0: Total_PCG])
        # Not accurate if we do not identify the primary node

        CompRes_Min = np.min(np.array([self.CompRes[self.state[i][0]] for i in range(Total_user)]))
